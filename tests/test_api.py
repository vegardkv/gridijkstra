...
class TestIntensityCosts(unittest.TestCase):
    def test_basic(self):
        ...
        src = np.array([2, 2])
        tar = np.array([8, 8])
        path, length = gridijkstra.plan(src, tar, costs, stencil)
    ...
    def test_ij_costs(self):
        ...
        src = np.array([9, 4])
        tar = np.array([2, 2])
        path, length = gridijkstra.plan(src, tar, bw, stencil)
    ...
    def test_rectangular_grid(self):
        ...
        src = np.array([1, 1])
        tar = np.array([3, 6])
        path, length, total = gridijkstra.plan(src, tar, costs, stencil, return_total_cost=True)
...
class TestAsymmetricCosts(unittest.TestCase):
    def test_basic(self):
        ...
        src = np.array([2, 2])
        tar = np.array([8, 8])
        path, length, total = gridijkstra.plan(src, tar, costs, return_total_cost=True)
        self.assertEqual(length, 12.0)
